...
#### Boosting ####
load("Scripts/RW Forecast/Long/Objects/h3-boosting05-env.RData")
load("Scripts/RW Forecast/Long/Objects/h6-boosting05-env.RData")
load("Scripts/RW Forecast/Long/Objects/h12-boosting05-env.RData")
...
for (i in 2:6) {
  ...
  gw[i] <- gw.test(m1$fc, mn$fc, length(test_Y), 721-(h-1), 1, alternative = "two.sided")$p.value
  gw2[i] <- gw.test(m1$fc, mn$fc, length(test_Y), 721-(h-1),1, alternative = "two.sided", power = 1)$p.value
}
...
for (i in 1:6) { 
  names <- paste0("Output/Plots/RW Forecast/Long H/",plotnames[i]," - h",h,".jpeg")
...
#save.image(file = "Scripts/RW Forecast/Long/Objects/h12-boosting05-env.RData")

#### Linear ####
load("Scripts/RW Forecast/Long/Objects/h3-linear05-env.RData")
load("Scripts/RW Forecast/Long/Objects/h6-linear05-env.RData")
load("Scripts/RW Forecast/Long/Objects/h12-linear05-env.RData")
...
m1_fc <- rwf_lm2(m1_data,0.5, norm = T, h = h)
m2_fc <- rwf_lm2(m2_data,0.5, norm = T, h = h)
m3_fc <- rwf_lm2(m3_data,0.5, norm = T, h = h)
m4_fc <- rwf_lm2(m4_data,0.5, norm = T, h = h)
m5_fc <- rwf_lm2(m5_data,0.5, norm = T, h = h)
m6_fc <- rwf_lm2(m6_data,0.5, norm = T, h = h)
...
#save.image(file = "Scripts/RW Forecast/Long/Objects/h12-linear05-env.RData")
